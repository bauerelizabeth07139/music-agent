  ...
  return (
    <div style={{ background: '#161b22', border: '1px solid #30363d', borderRadius: 8, padding: 16 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
        <span style={{ fontSize: 13, fontWeight: 600, color: '#e6edf3' }}>音轨配置</span>
      ...
      {tracks.map((t, i) => (
        <div key={t.id} style={{ display: 'flex', gap: 6, alignItems: 'center', marginBottom: 8, padding: '8px 10px', background: '#0d1117', border: '1px solid #30363d', borderRadius: 6, flexWrap: 'wrap' }}>
          <span style={{ fontSize: 11, color: '#8b949e', minWidth: 18 }}>#{i + 1}</span>
          <input type="text" value={t.name} onChange={e => onUpdate(t.id, 'name', e.target.value)}
            style={{ width: 80, padding: '4px 6px', background: '#161b22', border: '1px solid #30363d', borderRadius: 4, color: '#e6edf3', fontSize: 12 }} />
          <select value={t.role} onChange={e => onUpdate(t.id, 'role', e.target.value)}
            style={{ padding: '4px 6px', background: '#161b22', border: '1px solid #30363d', borderRadius: 4, color: '#e6edf3', fontSize: 12 }}>
          ...
          <span style={{ fontSize: 10, color: '#8b949e' }}>CH{t.channel}</span>
...
function RhythmEditor({ bpm, setBpm, timeSignature, setTimeSignature, songKey, setSongKey }) {
  return (
    <div style={{ background: '#161b22', border: '1px solid #30363d', borderRadius: 8, padding: 16 }}>
      <span style={{ fontSize: 13, fontWeight: 600, color: '#e6edf3', display: 'block', marginBottom: 12 }}>
        节奏与调性（参考，AI 可自行调整）
      </span>
      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
        <div style={{ flex: '1 1 120px' }}>
          <label style={{ fontSize: 11, color: '#8b949e', display: 'block', marginBottom: 4 }}>BPM</label>
          <input type="number" min="40" max="240" value={bpm} onChange={e => setBpm(parseInt(e.target.value) || 100)}
            style={{ width: '100%', padding: '6px 8px', background: '#0d1117', border: '1px solid #30363d', borderRadius: 4, color: '#e6edf3', fontSize: 13 }} />
        </div>
        <div style={{ flex: '1 1 120px' }}>
          <label style={{ fontSize: 11, color: '#8b949e', display: 'block', marginBottom: 4 }}>拍号</label>
        ...
        <div style={{ flex: '1 1 120px' }}>
          <label style={{ fontSize: 11, color: '#8b949e', display: 'block', marginBottom: 4 }}>调性</label>
...
function MetaBar({ arrangement }) {
  ...
  return (
    <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', fontSize: 12, color: '#8b949e', alignItems: 'center' }}>
      {arrangement.title && <span style={{ color: '#58a6ff', fontWeight: 600 }}>{arrangement.title}</span>}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      {plan.tracks.map((t, i) => {
        ...
        return (
          <div key={t.id} style={{ display: 'flex', gap: 12, alignItems: 'center', padding: '10px 14px', background: '#0d1117', border: '1px solid #30363d', borderRadius: 6 }}>
            <span style={{ fontSize: 18 }}>{t.role === 'vocal' ? '🎤' : '🎵'}</span>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 14, fontWeight: 600, color: '#e6edf3' }}>{t.name}</div>
              <div style={{ display: 'flex', gap: 8, marginTop: 4, flexWrap: 'wrap' }}>
                <span style={{ fontSize: 11, background: t.role === 'vocal' ? '#880e4f22' : '#1a237e22', color: t.role === 'vocal' ? '#f778ba' : '#58a6ff', padding: '2px 6px', borderRadius: 4 }}>
                  {t.role === 'vocal' ? '人声（TTS）' : '乐器（合成）'}
                </span>
                <span style={{ fontSize: 11, color: '#8b949e' }}>
                  {t.role === 'vocal' ? (t.voice || 'singing_female') : (t.instrument || 'piano')}
                </span>
                <span style={{ fontSize: 11, color: '#8b949e' }}>{t.part || 'melody'}</span>
                <span style={{ fontSize: 11, color: '#8b949e' }}>CH{t.channel ?? i}</span>
                {showNotes && noteCount > 0 && (
                  <span style={{ fontSize: 11, color: '#3fb950' }}>{noteCount} 音符</span>
                )}
                {!showNotes && (
...
export default function ArrangePanel({
  ...
  if (step === 'input') return (
    <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 16, flex: 1, overflow: 'auto' }}>
      ...
      <div>
        <label style={{ fontSize: 13, color: '#8b949e', display: 'block', marginBottom: 6 }}>创意描述</label>
        <textarea value={prompt} onChange={e => setPrompt(e.target.value)}
          placeholder="描述你想要的歌曲...例如：写一首关于星空和爱情的抒情流行歌曲"
          style={{ width: '100%', minHeight: 100, padding: 12, background: '#0d1117', border: '1px solid #30363d', borderRadius: 6, color: '#e6edf3', fontSize: 14, resize: 'vertical', fontFamily: 'inherit' }} />
      </div>
      <div style={{ display: 'flex', gap: 12, alignItems: 'flex-end', flexWrap: 'wrap' }}>
        <div style={{ flex: '1 1 200px' }}>
          <label style={{ fontSize: 13, color: '#8b949e', display: 'block', marginBottom: 6 }}>音乐风格</label>
          <select value={style} onChange={e => setStyle(e.target.value)}
            style={{ width: '100%', padding: '8px 10px', background: '#0d1117', border: '1px solid #30363d', borderRadius: 6, color: '#e6edf3', fontSize: 14 }}>
  ...
  if (step === 'lyrics-voice') return (
    <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 16, flex: 1, overflow: 'auto' }}>
      ...
      {lyricsData?.title && <div style={{ fontSize: 18, fontWeight: 600, color: '#58a6ff' }}>{lyricsData.title}</div>}
      {lyricsData?.sections && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {lyricsData.sections.map((s, i) => (
            <div key={i} style={{ background: '#161b22', border: '1px solid #30363d', borderRadius: 6, padding: 12 }}>
              <div style={{ fontSize: 12, color: '#8b949e', marginBottom: 6 }}>{s.label || s.type}</div>
              <pre style={{ margin: 0, whiteSpace: 'pre-wrap', fontSize: 14, color: '#e6edf3', fontFamily: 'inherit' }}>{s.lyrics}</pre>
            </div>
          ))}
        </div>
      )}
      <div>
        <label style={{ fontSize: 13, color: '#8b949e', display: 'block', marginBottom: 6 }}>完整歌词（可编辑）</label>
        <textarea value={confirmedLyrics} onChange={e => setConfirmedLyrics(e.target.value)}
          style={{ width: '100%', minHeight: 150, padding: 12, background: '#0d1117', border: '1px solid #30363d', borderRadius: 6, color: '#e6edf3', fontSize: 14, resize: 'vertical', fontFamily: 'inherit', lineHeight: 1.6 }} />
      </div>
      {references && references.results && references.results.length > 0 && (
        <div style={{ background: '#161b22', border: '1px solid #30363d', borderRadius: 8, padding: 12 }}>
          <div style={{ fontSize: 12, color: '#8b949e', marginBottom: 8 }}>参考信息</div>
          {references.results.slice(0, 3).map((r, i) => (
            <div key={i} style={{ fontSize: 12, color: '#58a6ff', marginBottom: 4 }}>
  ...
  if (step === 'plan-confirm' && arrangement) return (
    <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 16, flex: 1, overflow: 'auto' }}>
      ...
      <p style={{ fontSize: 13, color: '#3fb950', margin: 0 }}>✓ 人声已生成，编曲方案已基于人声时长自动规划</p>
  ...
  if (step === 'notes-confirm' && notesArrangement) {
    ...
    return (
      <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 16, flex: 1, overflow: 'auto' }}>
        ...
        <div style={{ background: '#161b22', border: '1px solid #30363d', borderRadius: 8, padding: 14, fontSize: 13, color: '#8b949e', lineHeight: 1.6 }}>
          <div style={{ fontWeight: 600, color: '#e6edf3', marginBottom: 6 }}>🔊 渲染说明</div>
          <div>• 人声轨道：歌词已嵌入音符，将通过 <span style={{ color: '#f778ba' }}>MiMo TTS</span> 合成真实演唱</div>
          <div>• 乐器轨道：音符已生成，将通过 <span style={{ color: '#58a6ff' }}>合成引擎</span> 并行渲染</div>
  ...
  if (step === 'generating') return (
    <div style={{ padding: 40, textAlign: 'center', color: '#8b949e' }}>
      ...
      <p style={{ fontSize: 16, color: '#e6edf3' }}>正在渲染音频...</p>
  ...
  return arrangement ? (
    <div style={{ padding: '12px 20px', background: '#0d1117', borderBottom: '1px solid #30363d', fontSize: 13, display: 'flex', gap: 16, flexWrap: 'wrap', alignItems: 'center' }}>
      <span style={{ color: '#58a6ff', fontWeight: 600 }}>{arrangement.title || '未命名'}</span>
      <span style={{ color: '#8b949e' }}>{arrangement.bpm || '?'} BPM</span>
      <span style={{ color: '#8b949e' }}>{arrangement.key || '?'}</span>
      <span style={{ color: '#8b949e' }}>{arrangement.tracks?.length || 0} 音轨</span>
